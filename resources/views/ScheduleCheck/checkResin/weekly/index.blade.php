@if ($schedulePlan->frequency_check->name == 'Weekly')
    @if (date('Y-m-d', strtotime($shiftDate)) >= $startOfWeekFormatted &&
            date('Y-m-d', strtotime($shiftDate)) <= $endOfWeekFormatted)

        @if (count(
                $schedulePlan->resin->schedule_records->whereBetween('shift_date', [$startOfWeekFormatted, $endOfWeekFormatted])->where('schedule_plan_id', $schedulePlan->id)) == 0)
            <div class="col-sm-6">
